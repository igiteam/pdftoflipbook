...
DEFAULT_IDENTIFIER="strategy-guides-archive-2024"
...
# Detect PDF and CBZ files in current directory using find with proper quoting
echo -e "\n${CYAN}🔍 Detecting PDF and CBZ files in current directory...${NC}"

# Use find to handle special characters properly
PDF_FILES=()
CBZ_FILES=()
...
# Show first 20 files with proper quoting
echo -e "${CYAN}📁 First 20 files:${NC}"
for i in "${!ALL_FILES[@]}"; do
    if [ $i -lt 20 ]; then
        # Use stat with proper quoting
        size=$(stat -c%s -- "${ALL_FILES[$i]}" 2>/dev/null || stat -f%z -- "${ALL_FILES[$i]}" 2>/dev/null || echo "0")
        size_human=$(numfmt --to=iec --suffix=B "$size" 2>/dev/null || echo "${size}B")
        ext="${ALL_FILES[$i]##*.}"
        echo "   $((i+1)). ${ALL_FILES[$i]} ($size_human) [$ext]"
    fi
done
...
# Calculate total size with proper quoting
total_size=0
...
def get_files() -> List[str]:
    ...
    for ext in ['.pdf', '.cbz']:
        try:
            ...
            print(f"Warning: Error reading directory: {e}")
    return sorted(files)
