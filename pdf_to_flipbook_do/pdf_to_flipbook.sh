...
ICON_URL="https://cdn.gitgpt.chat/rtx/images/pdf_flipbook.png"
...
class ViewController: NSViewController, NSDraggingDestination {
    ...
    @objc func convertPDF() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            for i in 0..<pageCount {
                ...
                print("Page \(pageNumber) file size: \(String(format: "%.2f", fileSizeMB)) MB")
            ...
            DispatchQueue.main.async {
                if successCount > 0 {
                    ...
                    self.generateCompleteHTML(pageCount: pageCount, title: sanitizedName, outputFolder: outputFolder)
    ...
    private func generateCompleteHTML(pageCount: Int, title: String, outputFolder: URL) {
        let html = """
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <script>
                (function () {
                    ...
                    function getSavedPage() {
                        const saved = localStorage.getItem('flipbook_last_page');
            ...
            `;
                    ...
                    function setupPageScrolling() {
                        ...
                        function endDrag() {
                            ...
                            const currentScroll = parseFloat(track.style.transform?.match(/translateY\\(([^)]+)/)?.[1] || 0);
                        ...
                        container.addEventListener('mousedown', (e) => {
                            ...
                            pageTrackStartScroll = transform && transform.includes('translateY') ?
                                parseFloat(transform.match(/translateY\\(([^)]+)/)?.[1] || 0) : 0;
                ...
                (function () {
                    ...
                    function handleWheel(e) {
                        ...
                        wheelTimeout = setTimeout(() => {
                            isScrolling = true;
                            const finalScroll = parseFloat(track.style.transform?.match(/translateY\\(([^)]+)/)?.[1] || 0);
        ...
        let htmlPath = outputFolder.appendingPathComponent("\(title)_flipbook.html")
        do {
            try html.write(to: htmlPath, atomically: true, encoding: .utf8)
            print("HTML saved to: \(htmlPath.path)")
        } catch {
